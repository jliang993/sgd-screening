...
for i_file = 1:8
    ...
    seed = randi(314159);
    rng(seed);
    rnd_idx = randi(m, max(3e2*m, 3.1e7)+10,1);
    %% lam_max
    lam_max = max(abs(A'*ones(m, 1))) /2;
    
    sS = [2; 2; 3; 3; 5; 10; 5; 20;];
    ss = sS(i_file);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    output_saga = output_xsol;
    output_saga.xsol = xsol;
    %% SGD setup
    para_sgd.xsol = xsol;
    para_sgd.type = type;
    ...
    % step-size
    Gamma = gammai/m ./ (ceil((1:maxits))).^0.51;
    ...
    [x_sgd, its_sgd, ek_sgd, dk_sgd, sk_sgd, tk_sgd] = ...
        func_proxSGD_SLR(para_sgd, rnd_idx);
    t_sgd = toc;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    output_sgd.x_sgd = x_sgd;
    output_sgd.its_sgd = its_sgd;
    output_sgd.ek_sgd = ek_sgd;
    output_sgd.dk_sgd = dk_sgd;
    output_sgd.sk_sgd = sk_sgd;
    output_sgd.tk_sgd = tk_sgd;
    %% SGD full-screen
    ...
    tic;
    [x_fs, its_fs, ek_fs,dk_fs, sk_fs,tk_fs] = ...
        func_proxSGD_SLR_fullscreen(para_sgd, rnd_idx);
    t_sgd_fs = toc;
    
    fprintf('      CPU-time: %.1fs...\n\n', t_sgd_fs);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    output_sgd_fs.x_fs = x_fs;
    output_sgd_fs.its_fs = its_fs;
    output_sgd_fs.ek_fs = ek_fs;
    output_sgd_fs.dk_fs = dk_fs;
    output_sgd_fs.sk_fs = sk_fs;
    output_sgd_fs.tk_fs = tk_fs;
    %% SGD online-screen
    ...
    tic;
    [x_os, its_os, ek_os,dk_os, sk_os,tk_os] = ...
        func_proxSGD_SLR_onlinescreen(para_sgd, rnd_idx);
    t_sgd_os = toc;
    
    fprintf('      CPU-time: %.1fs...\n\n', t_sgd_os);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    output_sgd_os.x_os = x_os;
    output_sgd_os.its_os = its_os;
    output_sgd_os.ek_os = ek_os;
    output_sgd_os.dk_os = dk_os;
    output_sgd_os.sk_os = sk_os;
    output_sgd_os.tk_os = tk_os;
    
    
    save([sprintf('SLR_%s.mat', filename)], ...
        'output_saga', 'output_sgd', 'output_sgd_fs', 'output_sgd_os');
    %% output....
    axesFontSize = 6;
