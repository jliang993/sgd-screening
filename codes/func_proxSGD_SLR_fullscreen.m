function [x, its, ek,dk, sk,tk] = func_proxSGD_SLR_fullscreen(para, rnd_idx)
...
while(its<maxits)
    ...
    if mod(its,T) ==0
        ...
        for j=1:m
            ...
            Pval = fac* ( logexp(-fj*x_vec(j))*m ) + (1-fac) * Pval;
            Dval = fac* - ( (abs(1 + c*vj*fj/m)>1e-14)* (1 + c*vj*fj/m) * log(1 + c*vj*fj/m) + ...
                (abs(-c*vj*fj/m)>1e-14)* (-c*vj*fj/m) * log(-c*vj*fj/m) )*m + (1-fac) * Dval ;
